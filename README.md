...
VEYDRIFT_WORKER_COUNT=<N>          # override the worker count (default: min(host CPU count, 2); set 1
                                  # to force the original single-process behavior). Values above the
                                  # max cap below are clamped so stale deploy env cannot reopen a
                                  # large reader pool accidentally.
VEYDRIFT_MAX_WORKER_COUNT=<N>      # optional safety cap for VEYDRIFT_WORKER_COUNT and the CPU-based
                                  # default. Defaults to 2; raise deliberately before running a larger
                                  # pool.
