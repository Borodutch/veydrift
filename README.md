...
VEYDRIFT_BURNING_CHICKEN_LEVEL_SELECTOR=<4-byte Chicken level getter selector, set to 0x05c58df2>
...
`VEYDRIFT_BURNING_CHICKEN_LEVEL_SELECTOR` is optional runtime metadata for the
Moon page Chicken level display; use `0x05c58df2` for the upgraded Chicken
contract unless the deployed helper changes.
...
project. Deploy this after the coordinate-carrying Chicken burn surface is in the
image. The listener then consumes
`ChickenBurned(address indexed burner,uint256 indexed tokenId,uint256 planetId,uint16 galaxy,uint16 system,uint8 position)`;
the Veydrift moon system validates those coordinates against current Veydrift
planet state before granting the moon. Configure it from the repository root
with:
