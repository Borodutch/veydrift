...
event must expose the burner, token id, and planet id by name. Coordinate fields are retained as
source evidence, while the Veydrift moon system still resolves canonical coordinates from
`planetId` before granting the moon and does not trust the event coordinates for authorization.

`ENABLE_TRANSFER_BURN_FALLBACK=true` also supports ERC-721 `Transfer(from, address(0), tokenId)`
burns when the burn transaction calldata is one of the documented `burn*ForMoon` shapes. Keep this
off for the upgraded Chicken contract, because
