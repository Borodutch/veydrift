...
export function PlayableMvpApp({ provider, readProvider, account, miniAppMode = false, planet }: PlayableMvpAppProps = {}) {
  ...
  const refreshBackendInfrastructureState = useCallback(async () => {
    if (!apiBaseUrl || !account) {
      setInfrastructureChainState(null);
      return null;
    }
  ...
  const runBuildingTransaction = useCallback(async (key: BuildingKey) => {
    await transactionActionGate.run(`building:start:${key}`, async () => {
      ...
      try {
        const latestInfrastructure = await refreshBackendInfrastructureState();
        const unavailableReason = refreshedInfrastructureUpgradeUnavailableReasonFor({
          buildingKey: key,
          gameContract,
          homePlanetId: planetId,
          infrastructureChainState: latestInfrastructure,
          isWalletConnected,
          onChainResources,
          runtimeConfigStatus: runtimeConfig.status,
        });
    ...
    provider,
    receiptProvider,
    refreshBackendInfrastructureState,
    refreshInfrastructureState,
    refreshOnChainState,
    runtimeConfig.status,
