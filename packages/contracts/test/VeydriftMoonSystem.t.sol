...
contract VeydriftMoonSystemTest is Test {
    ...
    function testSameParentPlanetMoonTransportUsesLocalDistance() public {
        uint256 planetId = _startPlanet();
        _createMoon(planetId);
        _fundPlanet(planetId, 20_000, 20_000, 20_000);
        _setShipCount(planetId, Ship.SmallCargo, 1);
        ...
        uint64 departureAt = uint64(block.timestamp);
        vm.prank(player);
        uint256 missionId = game.launchBodyFleetMission(
            planetId,
            planetId,
        ...
        uint256 expectedTravelSeconds =
            VeydriftAntiRaidPrimitives.travelSeconds(5, 5_000, 100, game.FLEET_UNIVERSE_SPEED());
