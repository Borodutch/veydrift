...
contract RejectingReferralInviter {
    ...
    function claimReferralCode(VeydriftReferralSystem referralSystem, string calldata code)
        external
    {
...
contract VeydriftGameTest is Test {
    ...
    function testReferralTopUpRequiresTwentyFourHoursAndKeepsPermanentCommitment() public {
        string memory code = "ref-1";
        ...
        (uint64 lastTopUpAt, uint64 nextTopUpAt, bool topUpAvailable) =
            referralSystem.referralTopUpState(player);
        assertEq(lastTopUpAt, firstClaimedAt);
        assertEq(nextTopUpAt, firstClaimedAt + 1 days);
        assertFalse(topUpAvailable);

        vm.warp(block.timestamp + 2 hours);
        vm.prank(player);
        vm.expectRevert(
            abi.encodeWithSelector(
                VeydriftReferralSystem.ReferralInviteTopUpUnavailable.selector,
                commitment,
                firstClaimedAt + 1 days
            )
        );
        referralSystem.claimReferralCode(code);
        ...
        (lastTopUpAt, nextTopUpAt, topUpAvailable) = referralSystem.referralTopUpState(player);
        assertEq(lastTopUpAt, firstClaimedAt + 1 days);
        assertEq(nextTopUpAt, firstClaimedAt + 2 days);
        assertFalse(topUpAvailable);

        assertEq(referralSystem.referralInvites(commitment), player);
        assertEq(referralSystem.referralCodeOwner(codeHash), player);
        assertTrue(referralSystem.referralCodeOwnedBy(player, codeHash));
    ...
    function testReferralClaimRequiresFirstPlanet() public {
        ...
        referralSystem.claimReferralCode("ref-no-planet");
    }

    function testReferralClaimAndTopUpFailClosedWhileGamePaused() public {
        vm.prank(player);
        game.startPlanet{value: 0.05 ether}();
        ...
        referralSystem.claimReferralCode("paused-code");
    ...
    function testReferralClaimRequiresCompletedMigration() public {
        VeydriftReferralSystem pendingReferralSystem = new VeydriftReferralSystem(admin);
        vm.prank(admin);
        pendingReferralSystem.setGame(address(game));

        vm.prank(player);
        game.startPlanet{value: 0.05 ether}();
        vm.prank(player);
        vm.expectRevert(VeydriftReferralSystem.ReferralMigrationPending.selector);
        pendingReferralSystem.claimReferralCode("pending");
    }

    function testReferralCodesNormalizeCaseAndRejectInvalidValues() public {
        vm.prank(player);
        game.startPlanet{value: 0.05 ether}();

        bytes32 normalizedHash = keccak256(bytes("my_code-1"));
        vm.prank(player);
        referralSystem.claimReferralCode("My_Code-1");
        assertEq(referralSystem.referralCodeOwner(normalizedHash), player);
        assertEq(referralSystem.normalizeReferralCode("My_Code-1"), "my_code-1");

        vm.warp(block.timestamp + 1 days);
        vm.prank(player);
        vm.expectRevert(VeydriftReferralSystem.ReferralCodeInvalid.selector);
        referralSystem.claimReferralCode("");
        vm.prank(player);
        vm.expectRevert(VeydriftReferralSystem.ReferralCodeInvalid.selector);
        referralSystem.claimReferralCode("contains space");
        vm.prank(player);
        vm.expectRevert(VeydriftReferralSystem.ReferralCodeInvalid.selector);
        referralSystem.claimReferralCode("abcdefghijklmnopqrstuvwxy");
    }

    function testReferralClaimPermanentlyRejectsCodeRotation() public {
        vm.prank(player);
        game.startPlanet{value: 0.05 ether}();

        string memory firstCode = "ref-active-1";
        string memory secondCode = "ref-active-2";
        bytes32 firstCodeHash = keccak256(bytes(firstCode));
        bytes32 secondCodeHash = keccak256(bytes(secondCode));
        ...
        referralSystem.claimReferralCode(firstCode);
        vm.expectRevert(
            abi.encodeWithSelector(
        ...
        referralSystem.claimReferralCode(secondCode);
        ...
        assertEq(referralSystem.referralCommitmentOf(player), firstCommitment);
        ...
        assertEq(referralSystem.referralInvites(secondCommitment), address(0));
        assertEq(referralSystem.referralCodeOwner(firstCodeHash), player);
        assertEq(referralSystem.referralCodeOwner(secondCodeHash), address(0));
    }

    function testReferralCodeCannotBeTakenOverByAnotherWalletDuringOrAfterExpiry() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                VeydriftReferralSystem.ReferralCodeAlreadyOwned.selector, codeHash, player
            )
        );
        referralSystem.claimReferralCode("REF-OWNED");
        ...
        assertEq(referralSystem.referralCommitmentOf(otherPlayer), bytes32(0));
        assertEq(referralSystem.referralCodeOwner(codeHash), player);
    }

    function testReferralMigrationRejectsCollisionAndPreservesExpiredOwnership() public {
        ...
        uint64[] memory activatedAts = new uint64[](1);
        bytes32[] memory legacyCommitments = new bytes32[](1);
        inviters[0] = player;
        codes[0] = "Legacy_Code";
        activatedAts[0] = uint64(block.timestamp - 2 days);
        bytes32 codeHash = keccak256(bytes("legacy_code"));
        legacyCommitments[0] = keccak256(bytes(codes[0]));
        bytes32 expectedValidHash = migrating.referralMigrationLeafValid(
            player, codeHash, legacyCommitments[0], activatedAts[0]
        );

        vm.prank(admin);
        migrating.configureReferralCodeMigration(expectedValidHash, 1, bytes32(0), 0);
        vm.prank(admin);
        migrating.migrateReferralCodes(inviters, codes, activatedAts, legacyCommitments);
        assertEq(migrating.referralCodeOwner(codeHash), player);

        inviters[0] = otherPlayer;
        legacyCommitments[0] = keccak256(bytes(codes[0]));
        vm.prank(admin);
        vm.expectRevert(
            abi.encodeWithSelector(
    ...
    function testReferralMigrationAcceptsRawReceiptCommitmentsAndKeepsNewBinding() public {
        VeydriftReferralSystem migrating = new VeydriftReferralSystem(admin);
        vm.warp(3 days);

        string memory validCode = "Legacy_Code";
        bytes32 validCodeHash = keccak256(bytes("legacy_code"));
        bytes32 validReceiptCommitment = keccak256(bytes(validCode));
        uint64 activatedAt = uint64(block.timestamp - 2 days);
        address hashOnlyOwner = address(0xBEEF);
        string memory hashOnlyCode = _legacyOverlengthCode(0);
        bytes32 hashOnlyCodeHash = keccak256(bytes(_lowercaseReferralCode(hashOnlyCode)));
        bytes32 hashOnlyReceiptCommitment = keccak256(bytes(hashOnlyCode));

        assertNotEq(validReceiptCommitment, migrating.referralCommitment(player, validCodeHash));
        assertNotEq(
            hashOnlyReceiptCommitment, migrating.referralCommitment(hashOnlyOwner, hashOnlyCodeHash)
        );

        bytes32 validManifestHash = migrating.referralMigrationLeafValid(
            player, validCodeHash, validReceiptCommitment, activatedAt
        );
        bytes32 hashOnlyManifestHash = migrating.referralMigrationLeafHashOnly(
            hashOnlyOwner, hashOnlyCodeHash, hashOnlyReceiptCommitment
        );
        vm.prank(admin);
        ...
        _importHashOnlyMigrationRow(
            migrating, hashOnlyOwner, hashOnlyCode, hashOnlyReceiptCommitment
        );
        vm.prank(admin);
        migrating.finalizeReferralCodeMigration();

        bytes32 canonicalCommitment = migrating.referralCommitment(player, validCodeHash);
        assertEq(migrating.referralCodeOwner(validCodeHash), player);
        assertEq(migrating.referralInvites(canonicalCommitment), player);
        assertEq(migrating.referralClaimedAt(canonicalCommitment), activatedAt);
        assertEq(migrating.referralCodeOwner(hashOnlyCodeHash), hashOnlyOwner);
        assertEq(migrating.referralCommitmentOf(hashOnlyOwner), bytes32(0));
        assertEq(migrating.referralInvites(hashOnlyReceiptCommitment), address(0));
    }

    function testReferralMigrationAcceptsCanonicalReplacementCommitment() public {
        VeydriftReferralSystem migrating = new VeydriftReferralSystem(admin);
        vm.warp(3 days);

        string memory code = "replacement-code";
        bytes32 codeHash = keccak256(bytes(code));
        bytes32 sourceCommitment = migrating.referralCommitment(player, codeHash);
        uint64 activatedAt = uint64(block.timestamp - 1 days);
        bytes32 manifestHash =
            migrating.referralMigrationLeafValid(player, codeHash, sourceCommitment, activatedAt);
    ...
    function testReferralMigrationFinalizesReviewedSixTenThreeInventoryOutOfOrder() public {
        ...
        address[] memory validOwners = new address[](6);
        string[] memory validCodes = new string[](6);
        uint64[] memory activatedAts = new uint64[](6);
        bytes32[] memory validCommitments = new bytes32[](6);
        validCodes[0] = "Legacy001";
        validCodes[1] = "legacy002";
        validCodes[2] = "legacy003";
        validCodes[3] = "legacy004";
        validCodes[4] = "legacy005";
        validCodes[5] = "legacy006";

        bytes32 validManifestHash;
        for (uint256 index = 0; index < validCodes.length; index++) {
            validOwners[index] = vm.addr(0x100 + index);
            activatedAts[index] = uint64(block.timestamp - (index + 2) * 1 days);
            bytes32 codeHash = keccak256(bytes(_lowercaseReferralCode(validCodes[index])));
            validCommitments[index] = keccak256(bytes(validCodes[index]));
            validManifestHash = validManifestHash
                ^ migrating.referralMigrationLeafValid(
                    validOwners[index], codeHash, validCommitments[index], activatedAts[index]
                );
        }

        address[] memory hashOnlyOwners = new address[](10);
        string[] memory hashOnlyCodes = new string[](10);
        bytes32[] memory hashOnlyCodeHashes = new bytes32[](10);
        bytes32[] memory hashOnlyCommitments = new bytes32[](10);
        string memory firstOverlengthCode;
        bytes32 hashOnlyManifestHash;
        for (uint256 index = 0; index < hashOnlyCodeHashes.length; index++) {
            hashOnlyOwners[index] = index == 0 ? player : vm.addr(0x200 + index);
            string memory overlengthCode = _legacyOverlengthCode(index);
            assertEq(bytes(overlengthCode).length, migrating.REFERRAL_LEGACY_CODE_LENGTH());
            if (index == 0) firstOverlengthCode = overlengthCode;
            hashOnlyCodes[index] = overlengthCode;
            hashOnlyCodeHashes[index] = keccak256(bytes(_lowercaseReferralCode(overlengthCode)));
            hashOnlyCommitments[index] = keccak256(bytes(overlengthCode));
            hashOnlyManifestHash = hashOnlyManifestHash
                ^ migrating.referralMigrationLeafHashOnly(
                    hashOnlyOwners[index], hashOnlyCodeHashes[index], hashOnlyCommitments[index]
                );
        }
        ...
        for (uint256 index = hashOnlyCodeHashes.length; index > 0; index--) {
            _importHashOnlyMigrationRow(
                migrating,
                hashOnlyOwners[index - 1],
                hashOnlyCodes[index - 1],
                hashOnlyCommitments[index - 1]
            );
        }
        for (uint256 index = validCodes.length; index > 0; index--) {
            _importValidMigrationRow(
                migrating,
                validOwners[index - 1],
                validCodes[index - 1],
                activatedAts[index - 1],
                validCommitments[index - 1]
            );
        }
        ...
        assertTrue(migrating.referralMigrationFinalized());
        assertEq(migrating.referralMigrationImportedValidCount(), 6);
        assertEq(migrating.referralMigrationImportedHashOnlyCount(), 10);
        assertEq(migrating.referralMigrationImportedValidHash(), validManifestHash);
        assertEq(migrating.referralMigrationImportedHashOnlyHash(), hashOnlyManifestHash);

        for (uint256 index = 0; index < validCodes.length; index++) {
            bytes32 codeHash = keccak256(bytes(_lowercaseReferralCode(validCodes[index])));
            assertEq(migrating.referralCodeOwner(codeHash), validOwners[index]);
            assertEq(
                migrating.referralCodeMigrationKind(codeHash),
                migrating.REFERRAL_MIGRATION_KIND_VALID()
            );
            bytes32 canonicalCommitment = migrating.referralCommitment(validOwners[index], codeHash);
            assertEq(migrating.referralClaimedAt(canonicalCommitment), activatedAts[index]);
        }
        for (uint256 index = 0; index < hashOnlyCodeHashes.length; index++) {
            assertEq(migrating.referralCodeOwner(hashOnlyCodeHashes[index]), hashOnlyOwners[index]);
            assertEq(
                migrating.referralCodeMigrationKind(hashOnlyCodeHashes[index]),
                migrating.REFERRAL_MIGRATION_KIND_HASH_ONLY()
            );
            bytes32 canonicalCommitment =
                migrating.referralCommitment(hashOnlyOwners[index], hashOnlyCodeHashes[index]);
            assertEq(migrating.referralInvites(canonicalCommitment), address(0));
            assertEq(migrating.referralClaimedAt(canonicalCommitment), 0);
        }

        bytes32 firstHashOnlyCodeHash = hashOnlyCodeHashes[0];
        bytes32 firstHashOnlyCommitment = hashOnlyCommitments[0];
        assertEq(migrating.referralCodeOwner(firstHashOnlyCodeHash), player);
        assertNotEq(keccak256(bytes(firstOverlengthCode)), firstHashOnlyCodeHash);
        assertEq(migrating.referralCodeOwner(keccak256(bytes(firstOverlengthCode))), address(0));
        assertTrue(migrating.referralCodeOwnedBy(player, firstHashOnlyCodeHash));
        assertEq(
            migrating.referralCodeMigrationKind(firstHashOnlyCodeHash),
            migrating.REFERRAL_MIGRATION_KIND_HASH_ONLY()
        );
        assertEq(migrating.referralCommitmentOf(player), bytes32(0));
        assertEq(migrating.referralInvites(firstHashOnlyCommitment), address(0));
        assertEq(migrating.referralClaimedAt(firstHashOnlyCommitment), 0);
        (address codeOwner,, uint64 activeUntil, bool active) =
            migrating.referralCodeState(firstHashOnlyCodeHash);
        assertEq(codeOwner, player);
        assertEq(activeUntil, 0);
        assertFalse(active);

        vm.prank(admin);
        migrating.setGame(address(game));
        vm.prank(player);
        game.startPlanet{value: 0.05 ether}();
        vm.prank(player);
        vm.expectRevert(VeydriftReferralSystem.ReferralCodeInvalid.selector);
        migrating.claimReferralCode(firstOverlengthCode);

        // Three JSON-only rows without successful receipts are deliberately absent.
        assertEq(migrating.referralCodeOwner(keccak256("json-only-unconfirmed-1")), address(0));
        assertEq(migrating.referralCodeOwner(keccak256("json-only-unconfirmed-2")), address(0));
        assertEq(migrating.referralCodeOwner(keccak256("json-only-unconfirmed-3")), address(0));
    }

    function testReferralMigrationPreservesReplayAndActiveCommitmentQuota() public {
        VeydriftReferralSystem migrating = new VeydriftReferralSystem(admin);
        vm.warp(10 days);
        vm.prank(admin);
        migrating.setGame(address(game));
        ...
        string memory code = "Shalex";
        bytes32 codeHash = keccak256(bytes("shalex"));
        bytes32 legacyCommitment = keccak256(bytes(code));
        bytes32 commitment = migrating.referralCommitment(player, codeHash);
        address invitee = address(0xCAFE10);
        uint64 activatedAt = uint64(block.timestamp - 1 hours);
        uint64 redeemedAt = uint64(block.timestamp - 30 minutes);
        bytes32 validManifestHash =
            migrating.referralMigrationLeafValid(player, codeHash, legacyCommitment, activatedAt);
        bytes32 redemptionManifestHash =
            migrating.referralMigrationLeafRedemption(player, invitee, commitment, redeemedAt);
        ...
        assertTrue(migrating.referralInviteeRedeemed(invitee));
        assertTrue(migrating.referralRedemptions(commitment, invitee));
        (uint8 remaining, uint64 nextRedemptionAt) = migrating.referralRedemptionQuota(commitment);
        assertEq(remaining, 2);
        assertEq(nextRedemptionAt, 0);

        vm.warp(activatedAt + 1 days);
        vm.prank(player);
        migrating.claimReferralCode("SHALEX");
        (remaining, nextRedemptionAt) = migrating.referralRedemptionQuota(commitment);
    ...
    function testReferralMigrationFailsClosedForRedemptionManifestAndInvalidRows() public {
        VeydriftReferralSystem migrating = new VeydriftReferralSystem(admin);
        vm.warp(10 days);
        string memory code = "preserve1";
        bytes32 codeHash = keccak256(bytes(code));
        bytes32 legacyCommitment = keccak256(bytes(code));
        bytes32 commitment = migrating.referralCommitment(player, codeHash);
        address invitee = address(0xCAFE11);
        uint64 activatedAt = uint64(block.timestamp - 1 hours);
        uint64 redeemedAt = uint64(block.timestamp - 30 minutes);
        bytes32 validManifestHash =
            migrating.referralMigrationLeafValid(player, codeHash, legacyCommitment, activatedAt);
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                VeydriftReferralSystem.ReferralRedemptionMigrationManifestMismatch.selector,
                redemptionManifestHash,
                bytes32(0),
                uint32(1),
                uint32(0)
            )
        ...
        _importRedemptionMigrationRow(
            migrating, player, invitee, commitment, uint64(block.timestamp + 1)
        );
        vm.expectRevert(
            abi.encodeWithSelector(
                VeydriftReferralSystem.ReferralMigrationRedemptionInvalid.selector,
        ...
        _importRedemptionMigrationRow(migrating, player, invitee, bytes32(uint256(123)), redeemedAt);
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                VeydriftReferralSystem.ReferralMigrationCountExceeded.selector, uint8(3), uint32(1)
            )
        );
        _importRedemptionMigrationRow(migrating, player, address(0xCAFE12), commitment, redeemedAt);
    }

    function testReferralMigrationRejectsWrongCommitmentAndCannotFinalizeWrongManifest() public {
        VeydriftReferralSystem migrating = new VeydriftReferralSystem(admin);
        vm.warp(3 days);
        string memory code = "receipt01";
        bytes32 codeHash = keccak256(bytes(code));
        uint64 activatedAt = uint64(block.timestamp - 1 days);
        bytes32 expectedCommitment = keccak256(bytes(code));
        bytes32 expectedManifestHash =
            migrating.referralMigrationLeafValid(player, codeHash, expectedCommitment, activatedAt);
        ...
        inviters[0] = player;
        codes[0] = code;
        activatedAts[0] = activatedAt;
        commitments[0] = bytes32(uint256(123));
        bytes32 canonicalCommitment = migrating.referralCommitment(player, codeHash);
        ...
        migrating.migrateReferralCodes(inviters, codes, activatedAts, commitments);

        address wrongOwner = address(0xBAD);
        inviters[0] = wrongOwner;
        commitments[0] = expectedCommitment;
        vm.prank(admin);
        migrating.migrateReferralCodes(inviters, codes, activatedAts, commitments);

        bytes32 wrongManifestHash =
            migrating.referralMigrationLeafValid(wrongOwner, codeHash, commitments[0], activatedAt);
        vm.prank(admin);
        vm.expectRevert(
            abi.encodeWithSelector(
                VeydriftReferralSystem.ReferralMigrationManifestMismatch.selector,
        ...
        migrating.finalizeReferralCodeMigration();
        assertFalse(migrating.referralMigrationFinalized());
    }

    function testReferralMigrationCannotFinalizeAlteredTimestamp() public {
        VeydriftReferralSystem migrating = new VeydriftReferralSystem(admin);
        vm.warp(3 days);
        string memory code = "receipt02";
        bytes32 codeHash = keccak256(bytes(code));
        bytes32 receiptCommitment = keccak256(bytes(code));
        uint64 reviewedActivatedAt = uint64(block.timestamp - 2 days);
        uint64 alteredActivatedAt = reviewedActivatedAt + 1;
        bytes32 expectedManifestHash = migrating.referralMigrationLeafValid(
            player, codeHash, receiptCommitment, reviewedActivatedAt
        );
        ...
        bytes32 alteredManifestHash = migrating.referralMigrationLeafValid(
            player, codeHash, receiptCommitment, alteredActivatedAt
        );
        vm.prank(admin);
        vm.expectRevert(
            abi.encodeWithSelector(
    ...
    function testReferralHashOnlyMigrationRejectsAlteredReceiptEvidence() public {
        VeydriftReferralSystem migrating = new VeydriftReferralSystem(admin);
        string memory code = _legacyOverlengthCode(0);
        bytes32 codeHash = keccak256(bytes(_lowercaseReferralCode(code)));
        bytes32 receiptCommitment = keccak256(bytes(code));
        bytes32 expectedManifestHash =
            migrating.referralMigrationLeafHashOnly(player, codeHash, receiptCommitment);

        vm.prank(admin);
        migrating.configureReferralCodeMigration(bytes32(0), 0, expectedManifestHash, 1);
        ...
        _importHashOnlyMigrationRow(migrating, player, code, bytes32(uint256(123)));

        string memory alteredCode = _legacyOverlengthCode(1);
        vm.expectRevert(
            abi.encodeWithSelector(
                VeydriftReferralSystem.ReferralMigrationCommitmentMismatch.selector,
                keccak256(bytes(alteredCode)),
                receiptCommitment
            )
        );
        _importHashOnlyMigrationRow(migrating, player, alteredCode, receiptCommitment);
    }

    function testReferralSettlementAcceptsPermanentInviteAfterTwentyFourHours() public {
        address invitee = address(0xCAFE);
        string memory code = "expired-code";
        bytes32 codeHash = keccak256(bytes(code));
        bytes32 commitment = referralSystem.referralCommitment(player, codeHash);
        vm.deal(invitee, 1 ether);
    ...
    function testReferralSettlementCreditsRejectingInviterWithoutBrickingInvitee() public {
        ...
        string memory code = "rejecting-code";
        bytes32 codeHash = keccak256(bytes(code));
    ...
    function testReferralSettleFirstPlanetCompatibilityPathPaysInviter() public {
        address invitee = address(0xBEEF);
        string memory code = "legacy-code";
        bytes32 codeHash = keccak256(bytes(code));
        bytes32 commitment = referralSystem.referralCommitment(player, codeHash);
        vm.deal(invitee, 1 ether);
        ...
        public
    {
        ...
        string memory code = "replay-code";
        bytes32 codeHash = keccak256(bytes(code));
        bytes32 commitment = referralSystem.referralCommitment(player, codeHash);
        vm.deal(invitee, 1 ether);
    ...
    function testReferralInviteeCannotReplayAfterTopUp() public {
        address invitee = address(0xCAFE);
        string memory code = "permanent-code";
        bytes32 commitment = referralSystem.referralCommitment(player, keccak256(bytes(code)));
        vm.deal(invitee, 1 ether);
        ...
        vm.prank(player);
        referralSystem.claimReferralCode(code);
        _startPlanetWithReferral(invitee, commitment);
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                VeydriftReferralSystem.ReferralInviteeAlreadyRedeemed.selector, commitment, invitee
            )
        );
        referralSystem.redeemReferralInvite{value: 0.025 ether}(invitee, commitment, v, r, s);
    }

    function testReferralSettlementAllowsThreeDistinctInviteesUntilExplicitTopUp() public {
        string memory code = "rolling-code";
        bytes32 codeHash = keccak256(bytes(code));
        bytes32 commitment = referralSystem.referralCommitment(player, codeHash);
        ...
        vm.warp(nextRedemptionAt + 1 days);
    ...
    function testReferralInviteRemainsRedeemableAfterTwentyFourHoursWithoutTopUp() public {
        string memory code = "permanent-code";
        bytes32 commitment = referralSystem.referralCommitment(player, keccak256(bytes(code)));
        address lateInvitee = address(0xCAFE5);
        vm.deal(lateInvitee, 1 ether);
        ...
        vm.warp(block.timestamp + 30 days);
        _startPlanetWithReferral(lateInvitee, commitment);

        (uint8 remaining,) = referralSystem.referralRedemptionQuota(commitment);
        assertEq(remaining, 2);
        (,,, bool active) = referralSystem.referralCodeState(keccak256(bytes(code)));
        assertTrue(active);
    }

    function testReferralTopUpAfterPartialUseResetsCapacityToExactlyThree() public {
        string memory code = "renewed-code";
        bytes32 codeHash = keccak256(bytes(code));
        bytes32 commitment = referralSystem.referralCommitment(player, codeHash);
        ...
        (remaining, nextRedemptionAt) = referralSystem.referralRedemptionQuota(commitment);
        assertEq(remaining, 2);
        assertEq(nextRedemptionAt, 0);
    ...
    function testInterplanetaryMissileAttackConsumesSilosInterceptionAndDestroysDefense() public {
        ...
        assertEq(uint8(missionType), uint8(VeydriftGameStorage.FleetMissionType.MissileAttack));
        assertEq(arrivalAt, departureAt + 270);
        assertEq(returnAt, arrivalAt);
    ...
    function testInterplanetaryMissileEmitsDefenseCountChangedForEveryLoss() public {
        ...
        // Launch only debits the silo; interception and impact emit when the flight arrives.
        vm.expectEmit(true, true, false, true, address(game));
    ...
    function testInterplanetaryMissilesResolveInScheduledArrivalOrder() public {
        (uint256 originPlanetId, uint256 targetPlanetId,) =
        ...
        _setPlanetCoordinates(targetPlanetId, 1, 110, 9);
        vm.prank(player);
        uint256 longRangeMission = game.launchInterplanetaryMissileAttack(
            originPlanetId, targetPlanetId, Defense.LightLaser, 1
        );
        _setPlanetCoordinates(targetPlanetId, 1, 101, 9);
        vm.prank(player);
        uint256 shortRangeMission = game.launchInterplanetaryMissileAttack(
            originPlanetId, targetPlanetId, Defense.LightLaser, 1
        );

        (, uint64 longArrival,,) = _fleetMission(longRangeMission);
        (, uint64 shortArrival,,) = _fleetMission(shortRangeMission);
        assertLt(shortArrival, longArrival);
        vm.warp(longArrival);
        vm.expectRevert(
            abi.encodeWithSelector(
                VeydriftGameStorage.FleetMissionNotResolved.selector, shortArrival
            )
        );
        game.resolveFleetMission(longRangeMission);

        game.resolveFleetMission(shortRangeMission);
        game.resolveFleetMission(longRangeMission);
    ...
    function testPlanetCannotBeAbandonedWhileIncomingMissileIsInFlight() public {
        ...
        _setShipCount(defenderHomePlanetId, Ship.ColonyShip, 1);
        uint256 targetPlanetId = _createResolvedColony(defender, defenderHomePlanetId, 83);
        _setPlanetCoordinates(targetPlanetId, 1, 104, 9);
        _setDefenseCount(originPlanetId, Defense.InterplanetaryMissile, 1);
        _setResources(targetPlanetId, 0, 0, 0);
        ...
        game.abandonPlanet(targetPlanetId);
    }

    function testInterplanetaryMissileFlightUsesClassicOGameTimingAndCannotBeRecalled() public {
        (uint256 originPlanetId, uint256 targetPlanetId,) =
            _seedMissileAttackPlanetsWithoutScoreProtection();
        _setTechnologyLevel(player, Technology.ImpulseDrive, 3);
        _setDefenseCount(originPlanetId, Defense.InterplanetaryMissile, 3);

        uint64 launchedAt = uint64(block.timestamp);
        _setPlanetCoordinates(targetPlanetId, 1, 100, 9);
        vm.prank(player);
        uint256 sameSystem = game.launchInterplanetaryMissileAttack(
            originPlanetId, targetPlanetId, Defense.RocketLauncher, 1
        );
        _setPlanetCoordinates(targetPlanetId, 1, 101, 9);
        vm.prank(player);
        uint256 oneSystem = game.launchInterplanetaryMissileAttack(
            originPlanetId, targetPlanetId, Defense.RocketLauncher, 1
        );
        _setPlanetCoordinates(targetPlanetId, 1, 110, 9);
        vm.prank(player);
        uint256 tenSystems = game.launchInterplanetaryMissileAttack(
            originPlanetId, targetPlanetId, Defense.RocketLauncher, 1
        );

        (, uint64 sameArrival, uint64 sameReturn,) = _fleetMission(sameSystem);
        (, uint64 oneArrival, uint64 oneReturn,) = _fleetMission(oneSystem);
        (, uint64 tenArrival, uint64 tenReturn,) = _fleetMission(tenSystems);
        assertEq(sameArrival, launchedAt + 30);
        assertEq(oneArrival, launchedAt + 90);
        assertEq(tenArrival, launchedAt + 630);
        assertEq(sameReturn, sameArrival);
        assertEq(oneReturn, oneArrival);
        assertEq(tenReturn, tenArrival);
        assertEq(game.activeFleetMissionCount(player), 0);
        ...
        game.recallFleetMission(sameSystem);
    }

    function testInterplanetaryMissileImpactUsesDefensesCompletedByArrivalNotLateResolution()
        public
    {
        (uint256 originPlanetId, uint256 targetPlanetId, address defender) =
            _seedMissileAttackPlanetsWithoutScoreProtection();
        ...
        uint256 transportOriginPlanetId = _createResolvedColony(defender, targetPlanetId, 91);
        _setPlanetCoordinates(transportOriginPlanetId, 1, 110, 8);
        _setShipCount(transportOriginPlanetId, Ship.SmallCargo, 1);
        _setResources(transportOriginPlanetId, 100_000, 100_000, 100_000);
        _setDefenseCount(originPlanetId, Defense.InterplanetaryMissile, 3);
        _setDefenseCount(targetPlanetId, Defense.LightLaser, 10);
        _setBuildingLevel(targetPlanetId, Building.Shipyard, 8);
        _setBuildingLevel(targetPlanetId, Building.NaniteFactory, 2);
        _setBuildingLevel(targetPlanetId, Building.MissileSilo, 2);
        ...
        // Two ABMs take 640 seconds here: one completes before the 630-second impact and one after.
        // A late resolver must snapshot the queue at impact, intercept with exactly one, and leave the
        // second ABM queued instead of retroactively letting it defend an already-arrived strike.
        vm.prank(defender);
        game.startDefenseProduction(targetPlanetId, Defense.AntiBallisticMissile, 2);
        VeydriftGameStorage.DefenseQueue memory queue = game.defenseQueue(targetPlanetId);
        assertEq(queue.readyAt, arrivalAt + 10);

        vm.prank(defender);
        uint256 transportMissionId = game.launchFleetMission(
            transportOriginPlanetId,
            targetPlanetId,
            VeydriftGameStorage.FleetMissionType.Transport,
            _smallCargoManifest(),
            VeydriftGameStorage.Resources({metal: 0, crystal: 0, deuterium: 0}),
    ...
    function testDueMissileResolvesBeforeDefenseHoldExpirySettlesLaterDefenses() public {
        (address ally, uint256 originPlanetId, uint256 targetPlanetId, uint256 allyPlanetId) =
            _seedDefenseHold();
        address defender = address(0xDEF);
        _setPlanetCoordinates(originPlanetId, 1, 100, 8);
        _setPlanetCoordinates(targetPlanetId, 1, 110, 9);
        _setPlanetCoordinates(allyPlanetId, 1, 110, 10);
        _setTechnologyLevel(player, Technology.ImpulseDrive, 3);
        _setDefenseCount(originPlanetId, Defense.InterplanetaryMissile, 3);
        _setDefenseCount(targetPlanetId, Defense.LightLaser, 10);
        ...
        uint256 holdMissionId = game.launchDefenseHold(
            allyPlanetId, targetPlanetId, defenders, _noCargo(), 100, 1 hours
        );
        (, uint64 holdArrivalAt,,) = _fleetMission(holdMissionId);
        uint64 holdUntil = holdArrivalAt + 1 hours;

        vm.prank(player);
        uint256 missileMissionId = game.launchInterplanetaryMissileAttack(
            originPlanetId, targetPlanetId, Defense.LightLaser, 3
        );
        (, uint64 missileArrivalAt,,) = _fleetMission(missileMissionId);
        assertLt(holdArrivalAt, missileArrivalAt);
        assertLt(missileArrivalAt, holdUntil);

        vm.prank(defender);
        game.startDefenseProduction(targetPlanetId, Defense.AntiBallisticMissile, 2);
        VeydriftGameStorage.DefenseQueue memory queue = game.defenseQueue(targetPlanetId);
        assertEq(queue.readyAt, missileArrivalAt + 10);

        vm.warp(holdUntil);
        vm.expectRevert(
            abi.encodeWithSelector(
                VeydriftGameStorage.FleetMissionNotResolved.selector, missileArrivalAt
            )
        );
    ...
    function testInterplanetaryMissileAttackUsesScoreProtectionButDoesNotCountBashing() public {
        ...
        uint256[] memory missionIds =
            new uint256[](VeydriftAntiRaidPrimitives.MAX_ATTACKS_PER_BASHING_WINDOW + 1);
        for (uint256 i = 0; i < VeydriftAntiRaidPrimitives.MAX_ATTACKS_PER_BASHING_WINDOW + 1; i++) {
            vm.prank(player);
            missionIds[i] = game.launchInterplanetaryMissileAttack(
                originPlanetId, targetPlanetId, Defense.LightLaser, 1
            );
        }
        ...
        public
    {
        ...
        assertEq(game.defenseCount(originPlanetId, Defense.InterplanetaryMissile), 0);
        assertEq(game.defenseCount(targetPlanetId, Defense.AntiBallisticMissile), 3);
        assertEq(game.defenseCount(targetPlanetId, Defense.PlasmaTurret), 2);
        (, uint64 arrivalAt,,) = _fleetMission(missionId);
        vm.warp(arrivalAt);
        vm.expectEmit(true, true, true, true, address(game));
        emit InterplanetaryMissileAttack(
    ...
    function testProductionBatchMixedCumulativeBudgetAndAtomicRollback() public {
        ...
        _setBuildingLevel(planetId, Building.Shipyard, 2);
        _setTechnologyLevel(player, Technology.CombustionDrive, 2);
        _setResources(planetId, 5_000, 5_000, 0);
        ProductionOrder[] memory orders = new ProductionOrder[](2);
        orders[0] = ProductionOrder(0, uint8(Ship.SmallCargo), 2);
        orders[1] = ProductionOrder(1, uint8(Defense.RocketLauncher), 2);
        vm.prank(player);
        vm.expectRevert(
            abi.encodeWithSelector(
                VeydriftGameStorage.InsufficientResources.selector, 1_000, 1_000, 0
            )
        );
        game.startProductionBatch(planetId, orders);
        assertFalse(game.shipQueue(planetId).active);
        assertFalse(game.defenseQueue(planetId).active);
        assertEq(game.planet(planetId).resources.metal, 5_000);

        orders[0].quantity = 1;
        orders[1].quantity = 1;
        vm.prank(player);
        game.startProductionBatch(planetId, orders);
        assertEq(game.shipQueue(planetId).quantity, 1);
        assertEq(game.defenseQueue(planetId).quantity, 1);
        assertEq(game.planet(planetId).resources.metal, 1_000);
        assertEq(game.planet(planetId).resources.crystal, 3_000);
        vm.prank(player);
        vm.expectRevert(
            abi.encodeWithSelector(
                VeydriftGameStorage.InsufficientResources.selector, 1_000, 3_000, 0
            )
        );
        game.startProductionBatch(planetId, orders);
    ...
    function testProductionBatchRejectsUnauthorizedPausedAndMalformed() public {
        vm.prank(player);
        uint256 planetId = game.startPlanet{value: 0.05 ether}();
        _setBuildingLevel(planetId, Building.Shipyard, 2);
        _setTechnologyLevel(player, Technology.CombustionDrive, 2);
        _setResources(planetId, 100_000, 100_000, 0);
        ProductionOrder[] memory orders = new ProductionOrder[](1);
    ...
    function testProductionBatchMeasuredAtLoadedBacklogs() public {
        ...
        emit log_named_uint("planet 4-order batch with 11+11 backlogs", used);
        assertLt(used, 12_000_000);
        assertEq(game.shipQueueBacklog(planetId).length, 13);
    ...
    function testProductionBatchDuplicateDomeRevertsAllChildQueues() public {
        ...
        _setTechnologyLevel(player, Technology.CombustionDrive, 2);
        _setResources(planetId, 5_000_000, 5_000_000, 5_000_000);
        ProductionOrder[] memory orders = new ProductionOrder[](3);
        orders[0] = ProductionOrder(0, uint8(Ship.SmallCargo), 1);
        orders[1] = ProductionOrder(1, uint8(Defense.SmallShieldDome), 1);
        orders[2] = ProductionOrder(1, uint8(Defense.SmallShieldDome), 1);
        vm.prank(player);
        vm.expectRevert(
            abi.encodeWithSelector(
                VeydriftGameStorage.DefenseLimitReached.selector, Defense.SmallShieldDome
        ...
        assertFalse(game.shipQueue(planetId).active);
        assertFalse(game.defenseQueue(planetId).active);
        assertEq(game.planet(planetId).resources.metal, 5_000_000);
    }

    function testProductionBatchRefusesFullShipBacklogWithoutAffectingPaidQueue() public {
        vm.prank(player);
        uint256 planetId = game.startPlanet{value: 0.05 ether}();
        _setBuildingLevel(planetId, Building.Shipyard, 2);
        _setTechnologyLevel(player, Technology.CombustionDrive, 2);
        _setResources(planetId, 1_000_000, 1_000_000, 0);
        vm.startPrank(player);
    ...
    function _importValidMigrationRow(
        VeydriftReferralSystem migrating,
        address inviter,
        string memory code,
        uint64 activatedAt,
        ...
        codes[0] = code;
        activatedAts[0] = activatedAt;
        legacyCommitments[0] = legacyCommitment;
        vm.prank(admin);
        migrating.migrateReferralCodes(inviters, codes, activatedAts, legacyCommitments);
    }

    function _importHashOnlyMigrationRow(
        VeydriftReferralSystem migrating,
        address inviter,
        string memory code,
        bytes32 legacyCommitment
        ...
        legacyCommitments[0] = legacyCommitment;
        vm.prank(admin);
        migrating.migrateLegacyReferralCodeOwnership(inviters, codes, legacyCommitments);
    }

    function _importRedemptionMigrationRow(
        VeydriftReferralSystem migrating,
        address inviter,
        address invitee,
        bytes32 commitment,
        ...
        redeemedAts[0] = redeemedAt;
        vm.prank(admin);
        migrating.migrateReferralRedemptions(inviters, invitees, commitments, redeemedAts);
    }

    function _legacyOverlengthCode(uint256 index) internal pure returns (string memory) {
        bytes memory code = bytes("abcdefghijklmnopqrstuvwxyz0123456789_-abcde");
        bytes10 digits = "0123456789";
        code[0] = "A";
        code[42] = digits[index];
        return string(code);
    }

    function _lowercaseReferralCode(string memory code) internal pure returns (string memory) {
        ...
        for (uint256 index = 0; index < source.length; index++) {
            ...
            if (character >= 65 && character <= 90) {
                normalized[index] = bytes1(character + 32);
            } else {
    ...
    function _newGame(address owner) internal returns (VeydriftGame) {
        ...
        vm.prank(owner);
        deployedReferralSystem.configureReferralCodeMigration(bytes32(0), 0, bytes32(0), 0);
        vm.prank(owner);
