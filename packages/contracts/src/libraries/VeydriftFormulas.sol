...
library VeydriftFormulas {
    ...
    function researchDuration(
        ...
        // Vanilla OGame research time in seconds at 1x universe speed:
        // (metal + crystal) / (1000 * (1 + effective lab level)) hours.
        // IRN is intentionally not applied until Veydrift supports multi-planet labs.
        uint256 raw =
            ((uint256(metalCost) + uint256(crystalCost)) * 1 hours) / (1000 * (labLevel + 1));
        return raw < minQueueSeconds ? minQueueSeconds : raw;
    }
