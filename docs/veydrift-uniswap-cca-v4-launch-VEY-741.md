...
It uses the audited v4-periphery settlement sequence—pre-fund PositionManager, `MINT_POSITION`, two
`SETTLE` actions with the official `1 << 255` contract-balance sentinel, then `TAKE_PAIR`—and returns
bounded unused inventory to the immutable recovery recipient. No ERC-20 or Permit2 allowance is left.
...
`packages/contracts/manifests/vey-741-base-fork-dry-run.json`. It enumerates official deployments,
the complete 1B allocation, token/WETH flows, auction recipients and outputs, v4 topology, full-range
resource ratios, reserve-release boundary, zero residual balances/allowances, and forbidden pools.
Validate it from `packages/contracts`:
...
The fork test deploys only fork-local VEYDRIFT/resource test tokens. Against the real official Base
deployments it creates the CCA, submits a Permit2-funded WETH bid, checkpoints and graduates, performs
automatic v4 migration, creates all three resource pools, verifies four NFTs in the immutable lock,
and verifies authority allowances are consumed. It never broadcasts.
...
| Token/approval leakage | Exact authority allowances; launcher-to-LBP approval zeroed; resource path uses direct PositionManager funding; all launcher balances returned/zero; postflight checks allowances |
