# Token launch: Uniswap CCA and v4

This guide describes the implementation and required Base-mainnet-fork proof. No deployment, funding,
auction registration, reserve release or liquidity creation is authorized by this document.
Any execution requires a newly approved schedule, fresh block bounds and every owner gate below.
Do not reuse an expired auction window. Supply, vesting and launch safeguards are maintained here;
alternative-venue scripts are excluded from this bundle.
...
UUPS upgrade. A compatible owner-upgradeable source implementation can perform one final upgrade into
this implementation after live verification; after that, no owner can install a mint-capable implementation. The proxies and
...
Before any irreversible operation, record the reviewed commit, chain, deployed proxy implementations,
owners, linked-library code hashes, storage compatibility, supplies and exact token balances.
Require an approved treasury, beneficiary addresses, release amounts and per-resource safety margins.
No placeholder or inferred deployer/beneficiary address is acceptable.

Simulate the exact authorized transaction sequence against the verified live state. Confirm:

- Resource supply remains unchanged by the no-mint upgrade, mint calls fail and further upgrades fail.
- VEYDRIFT supply, allocations, recipients, release start/end times and contributor cliff match approval.
- Game reserves cover internal liabilities and locked exits plus the approved margin after every release.
- Treasury receipt and game debit equal the exact approved resource amounts, with no unexpected deltas.
- Source/bytecode and linked libraries match the reviewed build; required implementations remain within EIP-170.
- Monitoring covers receipts, events, proxy slots, balances, liabilities and approvals before signing.

Follow [state preservation](open-alpha-state-preservation.md) and the current live-upgrade policy;
never use a token launch to bypass gameplay migration or rollback gates. Never export a Safe key to
run a script. Keep secrets out of source, logs, manifests and evidence.

### Irreversible boundaries

- A resource no-mint upgrade cannot be rolled back. Require independent review and fork proof first.
- After a reserve release but before liquidity creation, an approved treasury can return inventory
  through the game's deposit path; verify backing again. This is not automatic compensation.
- After liquidity creation, follow the recovery rules below. Do not unlock positions early,
  seed duplicate pools, replace token addresses, rewrite storage or mint compensating inventory.
- Any Game implementation rollback must preserve post-upgrade writes and pass state parity checks.

Price-dependent protocol accounting must use a separately reviewed TWAP/oracle design, never an AMM
spot price. User swaps require user-selected slippage. The launch's values, schedule, venue and risk
disclosures require explicit owner approval; no alternative venue is an automatic fallback.
...
```sh
BASE_MAINNET_RPC_URL="${BASE_MAINNET_RPC_URL:?Set the approved RPC}" bun run manifest:generate
...
3. Prove resource reserve backing and simulate any resource/game upgrade and exact reserve release
   first. The irreversible no-mint resource upgrade follows the [token foundation safeguards](#token-foundation).
...
7. Fund only the approved WETH bidder/capital path. CCA bidders use Permit2 and must receive normal
   ...
   and finalization readiness throughout the 86,400-block window.
...
| --- | --- |
| Auction misconfiguration | Exact 250M allocation; 86,400-block and step-sum checks; tick-boundary and floor checks; CREATE2 prediction; config hash; factory event and signed manifest |
